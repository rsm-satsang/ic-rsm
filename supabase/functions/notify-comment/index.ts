...
Deno.serve(async (req) => {
  ...
  try {
    const { projectId, commentId, commentText, authorId, recipientIds } = await req.json();
    ...
    const when = `${new Date().toLocaleString("en-US", { timeZone: "America/New_York", dateStyle: "medium", timeStyle: "short" })} EST`;
